...
async function buildClient(ctx) {
  ...
  const clientConfig = vite.mergeConfig(ctx.config, {
    ...
    base: "./",
    experimental: {
      renderBuiltUrl: (filename2, { type, hostType }) => {
  ...
  clientConfig.build.rollupOptions = defu(clientConfig.build.rollupOptions, {
    output: {
      assetFileNames: ctx.nuxt.options.dev ? void 0 : (chunk) => withoutLeadingSlash(join(ctx.nuxt.options.app.buildAssetsDir, `${sanitizeFilePath(filename(chunk.name))}.[hash].[ext]`)),
